...
import { ACTIVATION_CHALLENGE_TTL_MS, createActivationChallenge } from './activation.mjs';
import { createActivationManifest, serializeActivationManifest } from './activation-manifest.mjs';
import { CONTROL_REPOSITORY, CONTROL_WORKFLOW_PATH, CONTROL_DEFAULT_BRANCH,
  ISOLATED_RUNNER_GROUP } from './workflow-context-internal.mjs';
...
function runnerCreateArgs({ name, image, internal, suffix, runId, label, displaySocket, xauthorityFile, context }) {
  const args = ['create', '--name', name];
  containerLabels(args, suffix, runId, 'runner');
  args.push('--network', internal,
    ...
    '--env', `CONTROL_REPOSITORY_ID=${context.repositoryId}`,
    '--env', `CONTROL_EVENT_NAME=${context.eventName}`,
    '--env', `CONTROL_DEFAULT_BRANCH=${context.defaultBranch}`,
    '--env', `CONTROL_REF=${context.ref}`,
    '--env', `CONTROL_WORKFLOW_PATH=${context.workflowPath}`,
    '--env', `CONTROL_WORKFLOW_REF=${context.workflowRef}`,
    '--env', `CONTROL_RUN_ID=${context.runId}`,
    '--env', `CONTROL_RUN_ATTEMPT=${context.runAttempt}`,
    '--env', `CONTROL_WORKFLOW_SHA=${context.workflowSha}`,
    '--env', `CONTROL_CANDIDATE_SHA=${context.candidateSha}`,
    '--env', `CONTROL_ACTIVATION_COMMITMENT=${context.activationCommitment}`,
    '--env', `CONTROL_RUNNER_LABEL=${context.runnerLabel}`,
...
function verifyRunner(inspected, expected) {
  ...
  if (!variables.has(`DISPLAY=${DISPLAY}`) || !variables.has(`XAUTHORITY=${RUNNER_XAUTHORITY}`) ||
      ...
      !variables.has(`CONTROL_REPOSITORY_ID=${context.repositoryId}`) ||
      ...
      !variables.has(`CONTROL_WORKFLOW_PATH=${context.workflowPath}`) ||
      !variables.has(`CONTROL_WORKFLOW_REF=${context.workflowRef}`) ||
      !variables.has(`CONTROL_RUN_ID=${context.runId}`) ||
      !variables.has(`CONTROL_RUN_ATTEMPT=${context.runAttempt}`) ||
      !variables.has(`CONTROL_WORKFLOW_SHA=${context.workflowSha}`) ||
      !variables.has(`CONTROL_CANDIDATE_SHA=${context.candidateSha}`) ||
      !variables.has(`CONTROL_ACTIVATION_COMMITMENT=${context.activationCommitment}`) ||
      !variables.has(`CONTROL_RUNNER_LABEL=${context.runnerLabel}`) ||
...
function abortableActivationCallback(callback, args, signal, refusalCode) {
  let onAbort;
  const aborted = new Promise((_, reject) => {
    onAbort = () => reject(signal.reason ?? new RunnerSupervisorRefusal('runner_cancelled'));
    if (signal.aborted) onAbort();
    else signal.addEventListener('abort', onAbort, { once: true });
  });
  return Promise.race([Promise.resolve().then(() => callback(...args)), aborted]).catch((error) => {
    if (signal.aborted) {
      const reason = signal.reason;
      if (reason instanceof RunnerSupervisorRefusal) throw reason;
      refuse(reason?.code ?? 'runner_cancelled');
    }
    refuse(refusalCode);
  }).finally(() => signal.removeEventListener('abort', onAbort));
}

function normalizeRunSelectors(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) refuse('runner_activation_selector_invalid');
  let keys;
  try { keys = Reflect.ownKeys(value); } catch { refuse('runner_activation_selector_invalid'); }
  if (Object.getPrototypeOf(value) !== Object.prototype && Object.getPrototypeOf(value) !== null) {
    refuse('runner_activation_selector_invalid');
  }
  if (keys.length !== 2 || keys.some((key) => typeof key !== 'string') ||
      !keys.includes('runId') || !keys.includes('runAttempt')) refuse('runner_activation_selector_invalid');
  let runId;
  let runAttempt;
  try { ({ runId, runAttempt } = value); } catch { refuse('runner_activation_selector_invalid'); }
  if (typeof runId !== 'string' || !/^[1-9][0-9]{0,15}$/u.test(runId) ||
      !Number.isSafeInteger(Number(runId)) || String(Number(runId)) !== runId ||
      typeof runAttempt !== 'string' || !/^[1-9][0-9]{0,7}$/u.test(runAttempt) ||
      !Number.isSafeInteger(Number(runAttempt)) || String(Number(runAttempt)) !== runAttempt) {
    refuse('runner_activation_selector_invalid');
  }
  return Object.freeze({ runId, runAttempt });
}

function validateRunContext(context, selectors) {
  if (!context || typeof context !== 'object' || Array.isArray(context) ||
      context.repository !== CONTROL_REPOSITORY ||
      typeof context.repositoryId !== 'string' || !/^[1-9][0-9]{0,19}$/u.test(context.repositoryId) ||
      context.eventName !== 'workflow_dispatch' || context.defaultBranch !== CONTROL_DEFAULT_BRANCH ||
      context.ref !== `refs/heads/${CONTROL_DEFAULT_BRANCH}` || context.workflowPath !== CONTROL_WORKFLOW_PATH ||
      context.workflowRef !== `${CONTROL_REPOSITORY}/${CONTROL_WORKFLOW_PATH}@refs/heads/${CONTROL_DEFAULT_BRANCH}` ||
      context.runId !== selectors.runId || context.runAttempt !== selectors.runAttempt ||
      typeof context.workflowSha !== 'string' || !/^[a-f0-9]{40}$/u.test(context.workflowSha) ||
      context.runnerGroup !== ISOLATED_RUNNER_GROUP) {
    refuse('runner_workflow_context_invalid');
  }
  return context;
}

function contextFromManifest(manifest) {
  return Object.freeze({ repository: manifest.controlRepository,
    repositoryId: manifest.controlRepositoryId, eventName: manifest.eventName,
    defaultBranch: CONTROL_DEFAULT_BRANCH, ref: manifest.controlRef,
    workflowPath: manifest.controlWorkflowPath,
    workflowRef: `${manifest.controlRepository}/${manifest.controlWorkflowPath}@${manifest.controlRef}`,
    runId: manifest.runId, runAttempt: manifest.runAttempt, workflowSha: manifest.controlWorkflowSha,
    candidateSha: manifest.candidateSha, activationCommitment: manifest.activationCommitment,
    runnerLabel: manifest.runnerLabel,
    runnerGroup: ISOLATED_RUNNER_GROUP });
}

function sameManifest(actual, expected) {
  try {
    return serializeActivationManifest(actual) === serializeActivationManifest(expected);
  } catch {
    return false;
  }
}

export function createRunnerSupervisor({ readSelectedRunAttempt, verifyActivationAttestation,
  challengeFactory = createActivationChallenge, labelFactory = createAttemptRunnerLabel } = {}) {
  if (typeof readSelectedRunAttempt !== 'function' || typeof verifyActivationAttestation !== 'function' ||
      typeof challengeFactory !== 'function' || typeof labelFactory !== 'function') {
    refuse('runner_supervisor_config_invalid');
  }

  return async function runSupervisedRunner({ processBoundary, image, parentDisplay,
    parentXauthority, additionalEgressHosts = [], getRegistrationToken, presentActivation,
    selectRun, candidateSha, signal, timeoutMs = 45 * 60 * 1000 } = {}) {
    ...
    if (!processBoundary || typeof processBoundary.run !== 'function' || typeof processBoundary.start !== 'function' ||
        ...
        typeof presentActivation !== 'function' || typeof selectRun !== 'function' ||
        typeof candidateSha !== 'string' || !/^[a-f0-9]{40}$/u.test(candidateSha) ||
    ...
    try {
      ...
      challenge = challengeFactory({ candidateSha, runnerLabel: label });
    } catch { refuse('runner_activation_challenge_invalid'); }
    if (!challenge || typeof challenge.assertUsable !== 'function' || typeof challenge.consume !== 'function' ||
        typeof challenge.destroy !== 'function' || challenge.runnerLabel !== label ||
        challenge.candidateSha !== candidateSha || typeof challenge.commitment !== 'string' ||
        !/^[a-f0-9]{64}$/u.test(challenge.commitment) || !challenge.presentation ||
        Object.keys(challenge.presentation).sort().join(',') !== 'candidateSha,commitment,runnerLabel' ||
        challenge.presentation.commitment !== challenge.commitment ||
        challenge.presentation.runnerLabel !== label || challenge.presentation.candidateSha !== candidateSha) {
      try { challenge?.destroy?.(); } catch { /* fail closed */ }
      refuse('runner_activation_challenge_invalid');
    }

    try {
      challenge.assertUsable();
    } catch (error) {
      try { challenge.destroy(); } catch { /* fail closed */ }
      if (error?.code && /^activation_[a-z_]+$/u.test(error.code)) refuse(error.code);
      refuse('runner_activation_challenge_invalid');
    }
    ...
    const challengeTimeout = setTimeout(() => timerController.abort(
      new RunnerSupervisorRefusal('activation_challenge_expired')), ACTIVATION_CHALLENGE_TTL_MS);
    ...
    let xauthorityFile;
    try {
      await abortableActivationCallback(presentActivation, [challenge.presentation], operationSignal,
        'runner_activation_presentation_failed');
      challenge.assertUsable();
      const selectors = normalizeRunSelectors(await abortableActivationCallback(selectRun,
        [challenge.presentation, Object.freeze({ signal: operationSignal })], operationSignal,
        'runner_activation_selection_failed'));
      challenge.assertUsable();
      const runContext = validateRunContext(await readSelectedRunAttempt({ ...selectors, signal: operationSignal }), selectors);
      challenge.assertUsable();

      const expectedManifest = createActivationManifest({
        activationCommitment: challenge.commitment,
        candidateRepository: 'lawxcompany-stack/Plataforma-LawX',
        candidateSha,
        runnerLabel: label,
        controlRepository: runContext.repository,
        controlRepositoryId: runContext.repositoryId,
        controlRef: runContext.ref,
        controlWorkflowPath: runContext.workflowPath,
        runId: runContext.runId,
        runAttempt: runContext.runAttempt,
        controlWorkflowSha: runContext.workflowSha,
        eventName: runContext.eventName,
      });
      const verification = await verifyActivationAttestation({ manifest: expectedManifest, signal: operationSignal });
      if (operationSignal.aborted) refuse(operationSignal.reason?.code ?? 'runner_cancelled');
      challenge.assertUsable();
      const verifiedManifest = createActivationManifest(verification?.manifest);
      const expectedManifestDigest = createHash('sha256')
        .update(serializeActivationManifest(expectedManifest), 'utf8').digest('hex');
      if (!sameManifest(verifiedManifest, expectedManifest) ||
          verification.manifestDigest !== expectedManifestDigest ||
          verifiedManifest.activationCommitment !== challenge.commitment ||
          verifiedManifest.candidateSha !== challenge.candidateSha ||
          verifiedManifest.runnerLabel !== challenge.runnerLabel ||
          verifiedManifest.runId !== selectors.runId || verifiedManifest.runAttempt !== selectors.runAttempt) {
        refuse('activation_attestation_invalid');
      }

      // Zero the nonce before any parent-display, Docker, token or runner access.
      challenge.consume();
      trusted = contextFromManifest(verifiedManifest);
      label = verifiedManifest.runnerLabel;
      suffix = suffixFromLabel(label);
      ...
      try { challenge.destroy(); } catch { /* cleanup below is fail-closed */ }
      if (tempDirectory) {
      ...
      if (error instanceof RunnerSupervisorRefusal) throw error;
      if (error?.code && /^(?:activation|egress|runner)_[a-z_]+$/u.test(error.code)) {
        throw new RunnerSupervisorRefusal(error.code);
      }
      refuse('runner_activation_refused');
    }
  let displayProcess;
  let failure;
  let exitCode;
  ...
  try {
    ...
    const runnerId = parseDockerId(await dockerCommand(processBoundary, runnerCreateArgs({
      name: names.runner, image, internal: names.internal, suffix, runId, label,
      displaySocket: DISPLAY_SOCKET, xauthorityFile, context: trusted,
    ...
    signal?.removeEventListener('abort', onExternalAbort);
    token = undefined;
    try { challenge.destroy(); } catch { /* cleanup continues closed */ }
